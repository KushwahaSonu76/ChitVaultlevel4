...
impl ChitVaultContract {
    ...
    pub fn contribute(env: Env, chit_id: u32, round: u32, member: Address) {
        ...
        let chit: ChitStatus = env
            .storage()
            .instance()
            .get(&DataKey::Chit(chit_id))
            .expect("Chit not found");
        ...
        env.storage()
            .instance()
            .set(&DataKey::Contributions(chit_id, round), &contributions);
    ...
    pub fn disburse(env: Env, chit_id: u32, round: u32) {
        ...
        if chit.current_round < chit.total_rounds {
            ...
            env.storage().instance().set(
                &DataKey::Contributions(chit_id, chit.current_round),
                &empty_contributions,
            );
        } else {
